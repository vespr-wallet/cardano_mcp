 ...
 *
 * Tests all 11 endpoints for:
 *   - Baseline response time (sequential, < 2s threshold)
...
async function runConcurrencyTest(
  ...
  return {
    ...
    throughputRps: Math.round((tasks.length / wallTimeMs) * 1000),
  };
}
...
async function runIndividualTests(): Promise<TestResult[]> {
  ...
  const tests: Array<[string, () => Promise<unknown>]> = [
    ...
    ["get_wallet_balance",      () => VesprApiRepository.getDetailedWallet(TEST_CONFIG.address)],
    ["get_transaction_history", () => VesprApiRepository.getTransactionHistory(TEST_CONFIG.address)],
    ["get_staking_info",        () => VesprApiRepository.getStakingInfo(TEST_CONFIG.address)],
    ...
    ["get_trending_tokens",     () => VesprApiRepository.getTrendingTokens(FiatCurrency.USD, "1H")],
    ["resolve_ada_handle",      () => VesprApiRepository.resolveAdaHandle(TEST_CONFIG.handle)],
    ["get_asset_metadata",      () => VesprApiRepository.getAssetMetadata(TEST_CONFIG.tokenUnit)],
    ["get_asset_summary",       () => VesprApiRepository.getAssetSummary([TEST_CONFIG.tokenUnit, TEST_CONFIG.vesprUnit])],
  ];
...
async function runConcurrencyTests(): Promise<ConcurrencyResult[]> {
  ...
  // Scenario D: 20 parallel — all tools mixed (diverse cache keys, no overlap)
  results.push(
    await runConcurrencyTest(
      `all tools mixed × ${CONCURRENT_REQUESTS} parallel (diverse keys)`,
      [
        () => VesprApiRepository.getDetailedWallet(TEST_CONFIG.address),
        () => VesprApiRepository.getTransactionHistory(TEST_CONFIG.address),
        () => VesprApiRepository.getStakingInfo(TEST_CONFIG.address),
        () => VesprApiRepository.getTokenInfo(TEST_CONFIG.tokenUnit, FiatCurrency.USD),
        () => VesprApiRepository.getTokenInfo(TEST_CONFIG.tokenUnit, FiatCurrency.EUR),
        () => VesprApiRepository.getTokenChart(TEST_CONFIG.tokenUnit, "24H", CryptoCurrency.ADA),
        () => VesprApiRepository.getTokenChart(TEST_CONFIG.tokenUnit, "1W", CryptoCurrency.ADA),
        () => VesprApiRepository.getTrendingTokens(FiatCurrency.USD, "1H"),
        () => VesprApiRepository.getTrendingTokens(FiatCurrency.EUR),
        () => VesprApiRepository.resolveAdaHandle(TEST_CONFIG.handle),
        () => VesprApiRepository.getAssetMetadata(TEST_CONFIG.tokenUnit),
        () => VesprApiRepository.getAssetMetadata(TEST_CONFIG.vesprUnit),
        () => VesprApiRepository.getAssetSummary([TEST_CONFIG.tokenUnit, TEST_CONFIG.vesprUnit]),
        () => VesprApiRepository.getAdaSpotPrice(FiatCurrency.USD),
        () => VesprApiRepository.getAdaSpotPrice(FiatCurrency.EUR),
        () => VesprApiRepository.getAdaSpotPrice(FiatCurrency.GBP),
        () => VesprApiRepository.getDetailedWallet(TEST_CONFIG.address),
        () => VesprApiRepository.getTokenInfo(TEST_CONFIG.vesprUnit, FiatCurrency.USD),
        () => VesprApiRepository.getTrendingTokens(FiatCurrency.GBP),
      ],
    ),
  );
